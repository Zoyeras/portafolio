...
const ProjectsSection = ({ language }: { language: Language }) => {
  ...
  const copy = {
    es: {
      ...
      title: "Mis", titleHighlight: "Proyectos",
      subtitle: "Algunos de mis trabajos recientes donde he aplicado mis habilidades para resolver problemas reales.",
      close: "Cerrar", viewDemo: "Ver Demo", viewFrontend: "Ver Frontend",
      viewBackend: "Ver Backend", viewGuide: "Ver Guía", viewDetails: "Ver detalles",
    },
    en: {
      eyebrow: "Recent Work",
      title: "My", titleHighlight: "Projects",
      subtitle: "Some of my recent work where I applied my skills to solve real-world problems.",
      close: "Close", viewDemo: "View Demo", viewFrontend: "View Frontend",
      viewBackend: "View Backend", viewGuide: "View Guide", viewDetails: "View details",
    },
  };
  ...
  const projects = [
    {
      title: { es: "Gestor de Tareas", en: "Task Manager" },
      description: { es: "Aplicación web para gestión de tareas con sistema de autenticación, roles de usuario y administrador, y tablero de tareas interactivo.", en: "Web app for task management with authentication, user/admin roles, and an interactive task board." },
      image: "/imagenes/Capturas Proyectos/imgGestorTareas.png",
      technologies: ["React", "Python", "Flask", "MySQL", "JWT"],
      details: {
        es: `<p>Sistema completo de gestión de tareas con:</p><ul><li>Autenticación JWT segura</li><li>Roles de usuario y administrador</li><li>Tablero Kanban interactivo</li><li>Notificaciones en tiempo real</li><li>Reportes y estadísticas</li></ul>`,
        en: `<p>Complete task management system with:</p><ul><li>Secure JWT authentication</li><li>User and administrator roles</li><li>Interactive Kanban board</li><li>Real-time notifications</li><li>Reports and analytics</li></ul>`,
      },
      demoUrl: "https://gestortareas-frontend.vercel.app/login",
      frontendRepoUrl: "https://github.com/Zoyeras/backend-GestorTareas",
      backendRepoUrl: undefined, guideUrl: undefined,
    },
    {
      title: { es: "Programa de automatización", en: "Automation Program" },
      description: { es: "Programa de automatización para creación de tickets con interfaz de usuario, toma de datos con formulario y envíos de WhatsApp.", en: "Automation program for ticket creation with UI, form-based data capture, and WhatsApp sending." },
    ...
    {
      title: { es: "Ecommerce ligero", en: "Light Ecommerce" },
      description: { es: "Catálogo e-commerce conversacional administrable desde Google Drive. Los productos se actualizan en tiempo real, el carrito y checkout se realizan por WhatsApp.", en: "Conversational e-commerce catalog managed via Google Drive. Products update in real time, cart and checkout via WhatsApp." },
      image: "/imagenes/Capturas Proyectos/imgEcommerceLigero.png",
      technologies: ["React", "TypeScript", "Vite", "Google Drive API", "Vercel"],
      details: {
        es: `<p>Demo de tienda en línea con:</p><ul><li>Catálogo dinámico desde Google Drive</li><li>Filtro por categoría y carga por lotes</li><li>Carrito con total estimado</li><li>Checkout por WhatsApp</li><li>Vista previa modal de productos</li><li>Tema oscuro/claro persistente</li><li>API serverless en Vercel</li></ul>`,
        en: `<p>Demo online store featuring:</p><ul><li>Dynamic catalog from Google Drive</li><li>Category filter and batch loading</li><li>Shopping cart with estimated total</li><li>WhatsApp checkout</li><li>Product modal preview</li><li>Persistent dark/light mode</li><li>Serverless API on Vercel</li></ul>`,
      },
      demoUrl: "https://ecommerce-ligero.vercel.app",
      frontendRepoUrl: "https://github.com/Zoyeras/Ecommerce-Ligero",
      backendRepoUrl: undefined,
      guideUrl: "https://github.com/Zoyeras/Ecommerce-Ligero/blob/main/GESTION.md",
    },
    {
      title: { es: "TaskManager", en: "TaskManager" },
      description: { es: "Aplicación completa de gestión de tareas con autenticación JWT, roles de usuario y administrador, desarrollada con .NET 10 (backend) y React + TypeScript (frontend).", en: "Full-stack task management app with JWT authentication, user/admin roles, built with .NET 10 (backend) and React + TypeScript (frontend)." },
  ...
  const getLinks = (p: typeof projects[0]): LinkBtn[] => {
    const links: LinkBtn[] = [];
    if (p.demoUrl && p.demoUrl !== "#") links.push({ href: p.demoUrl, label: text.viewDemo, variant: "primary" });
    if (p.frontendRepoUrl) links.push({ href: p.frontendRepoUrl, label: text.viewFrontend, variant: "secondary" });
    if (p.backendRepoUrl) links.push({ href: p.backendRepoUrl, label: text.viewBackend, variant: "secondary" });
    if (p.guideUrl) links.push({ href: p.guideUrl, label: text.viewGuide, variant: "ghost" });
    return links;
  };

  const btnStyle: Record<Variant, string> = {
    primary: "bg-green-400 text-black hover:bg-green-300 shadow-md shadow-green-900/20",
    secondary: "bg-purple-800/70 text-purple-100 border border-purple-600/50 hover:border-purple-400/70 hover:bg-purple-700/70",
    ghost: "bg-transparent text-purple-400 border border-purple-700/35 hover:border-purple-500/55 hover:text-purple-200",
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-4 reveal">
          ...
          <h2 className="text-4xl md:text-5xl font-black tracking-tight section-title-glow inline-block" style={{ fontFamily: "var(--font-display)" }}>
            ...
            <span className="text-white heading-underline">{text.titleHighlight}</span>
          </h2>
        </div>
        ...
        {/* Grid */}
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-5 ${projects.length === 3 ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
          {projects.map((project, index) => (
            <div key={index} className={`reveal-scale ${staggerDelays[index] ?? ""}`}>
              <ProjectCard
      ...
      {proj !== null && selectedProject !== null && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setSelectedProject(null)}>
          ...
          <div
            ...
            style={{ background: "linear-gradient(160deg, rgb(13 8 32 / 0.98), rgb(5 2 15 / 0.99))" }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Imagen hero del modal */}
            <div className="relative h-56 md:h-68 overflow-hidden rounded-t-2xl">
              <img src={proj.image} alt={proj.title[language]} className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0d0820] via-black/35 to-transparent" />

              {/* Título sobre imagen */}
              <div className="absolute bottom-0 left-0 p-6">
                <p className="text-xs font-semibold tracking-[0.2em] uppercase text-green-400/65 mb-1" style={{ fontFamily: "var(--font-display)" }}>
                  {language === "es" ? "Proyecto" : "Project"}
                </p>
                <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight leading-tight" style={{ fontFamily: "var(--font-display)" }}>
                  {proj.title[language]}
                </h3>
              </div>
            ...
            <div className="p-6 md:p-8 space-y-6">
              {/* Detalles */}
              <div className="modal-details text-sm font-light leading-relaxed"
              ...
              {/* Tech */}
              <div>
                <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-purple-500/55 mb-3" style={{ fontFamily: "var(--font-display)" }}>
                  Tech Stack
                </p>
